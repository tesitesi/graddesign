...
cccalcS5 = readmatrix('/Users/tesiyosi/dev/graddesign/cool_matlab/cool_matlab_input.csv');

gamma_g = 1.126;  %ƒKƒX”ä”M”ä
...
Rt = 0.062; %ƒXƒ[ƒg”¼Œa [m]
Dt = Rt * 2;     %ƒXƒ[ƒgŒa [m]
At = Rt^2 * pi;  %ƒXƒ[ƒg–ÊÏ
Tc = 3636.42;    %ƒ`ƒƒƒ“ƒo‰·“x [K]
g = 9.80655; %d—Í‰Á‘¬“x[m/s^2]
Pc = 6.0 *10^6;  %ƒ`ƒFƒ“ƒoˆ³—Í[Pa]
Gc = 10.30; %—â‹pÜ—¬—Ê[kg/s]
Gg = 41.19;   %„iÜ—¬—Ê[kg/s]
c = Pc * At / Gg;   %“Á«”r‹C‘¬“x[m/s]
%lambda = 390;    %”M“`“±—¦[W/mK]
lambda_cond = 0.100;  %—â‹pÜ”M’ïR[W/m*K]
...
uc = cccalcS5(:,14); %—â‹pÜ—¬‘¬
Dc = cccalcS5(:,15); %—â‹pÜ—¬˜H“™‰¿’¼Œa
lambda = cccalcS5(:,20);
Dp = cccalcS5(:,21);

throat = 286;   %ƒXƒ[ƒgˆÊ’u
L = size(x);
L = L(1);
