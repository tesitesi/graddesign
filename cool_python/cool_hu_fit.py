...
def u(T):
    ...
    opt, _ = curve_fit(f, t_sample, u_sample)
    print(opt)
    u =  opt[0] / (T + opt[1]) + opt[2]
    return u
...
def lambdal(T):
    ...
    sigmal_sample = sigmal_sample * 0.0009478171208703 / 39.3701 /  (9./5.) # [(Btu/lb F )* lb/in sec] = [Btu / F in sec]
